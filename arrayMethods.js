...
// CODE HERE
const totalPopulation = populations.reduce(function(runningTotal, curIndex){
  return runningTotal + curIndex
});
...
const myStrongestMonster = monstersInYourPocket.filter(function(monster){
  return monster.CP > 200
})

console.log(myStrongestMonster)
...
// CODE HERE
const totalOrders = orders.map(function(order){
...
let bobsTotal = purchases.filter(function(purchase){
  return purchase.owner === 'Bob'
}).reduce(function(acc, purchase){

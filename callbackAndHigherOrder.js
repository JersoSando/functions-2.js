...
uniq(names, (UniqArrWithName) => {
  console.log('The new names array with all the duplicate items removed is', UniqArrWithName);
});
...
const each = (arr, cb) => {
  arr.forEach((elem, indx) => cb(elem, indx))
};
...
const getUserById = (arr, id, cb) => {
  const foundObj = arr.filter(obj => obj.id === id)[0];
...
getUserById(users, '16t', user => {
  console.log('The user with the id 16t has the email of ' + user.email + ' the name of ' + user.name + ' and the address of ' + jersoUser.address) 
})
